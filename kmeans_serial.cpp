...
public:
	...
	vector<double> run(vector<Point> & points)
	{
...
void benchmark(int K, int total_points, int total_values, int max_iterations, vector<Point> points, int num_runs) {
    double total_exec, phase1_exec, phase2_exec, iters = 0.0; 
    for (size_t i = 0; i < num_runs; ++i) 
    {
        KMeans kmeans_test(K, total_points, total_values, max_iterations);
        vector<double> results = kmeans_test.run(points);
    ...
    cout << "\nAverage Results of " << num_runs << " runs: " << endl;
...
int main(int argc, char *argv[])
{
    ...
    benchmark(K, total_points, total_values, max_iterations, points, 5);
	return 0;
}
